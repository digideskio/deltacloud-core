...
class CIMI::Service::MachineNetworkInterface < CIMI::Service::Base
    ...
    if id == :all
      ...
      volumes.inject([]) do |attached, vol|
        ...
        attached <<  self.new(context, :values => {
          ...
          :description => "MachineNetworkInterface #{vol.id} for Machine #{instance_id}",
  ...
  def self.collection_for_instance(instance_id, context)
    ...
    CIMI::Model::MachineNetworkInterface.list(volumes_url, machine_volumes, :add_url => attach_url)
  end
